...
__global__ void radix_sort_iter_get_bits(unsigned int* input unsigned int* bits, unisgned int n, unsigned int iter) {
    ...
    if (i < n) {
        ...
        bit = (key >> iter) & 1;
        bits[i] = bit;
        atomicAdd(blockPositions[blockIdx + (bit == 0) ? blockIdx : 0], 1);
    }
    ...
    if (i < n) {
        for (unsigned int stride = 1; stride < blockDim.x; stride *= 2) {
            ...
            if (threadIdx.x >= stride) {
                temp = bits[threadIdx.x] + bits[threadIdx.x - stride];
            }
            __syncthreads();
            if (threadIdx.x >= stride) {
                bits[threadIdx.x] = temp;
            }
        }
        if (i < n) {
            exclsuiveScannedBits[i] = bits[threadIdx.x];
        }
    }
...
__global__ void radix_sort_iter_assign(unsigned int* input, unsigned int* output, unsigned int* blockPositions, unisgned int n) {
    unsigned int i = threadIdx.x;
    if (i < n) {
        unsigned int dst = (bit == 0) ? (blockPositions[blockIdx.x + (n / SECTION_SIZE)] + i) : (blockPositions[blockIdx.x.x] + i);
